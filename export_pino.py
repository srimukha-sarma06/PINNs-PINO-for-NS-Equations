...
layer_size_branch = [64, 64]
layer_size_trunk = [64, 64]
...
class Deeponet(tf.keras.Model):
    def __init__(self, layer_sizes_branch, layer_sizes_trunk):
        ...
        self.branch_network = tf.keras.Sequential([
            tf.keras.layers.Dense(layer, activation="tanh") for layer in layer_sizes_branch
        ])

        self.trunk_network = tf.keras.Sequential([
            tf.keras.layers.Dense(layer, activation="tanh") for layer in layer_sizes_trunk
        ])
...
deeponet.load_weights("PINO_TUTORIAL/pino_models_tf_fixed/pino_01_fixed-20293.weights.h5")
